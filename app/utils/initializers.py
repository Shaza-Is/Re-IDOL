...
def create_data(files: List[str]) -> DataFrame:
    ...
    for file in files: 
        ...
        mag_data = df[["iphoneMagX", "iphoneMagY", "iphoneMagZ"]] 
        mag_data = min_max(mag_data)
...
def min_max(mag_data: np.ndarray):
    """min_max applies min_max scaling to the magnetometer data
    in order to simulate ellipsoid fitting. 

    Args:
        mag_data (np.ndarray): a 3xn matrix with magnetometer data
    """
    scaler = MinMaxScaler()
    result = scaler.transform_data()
